...
module case_1(
	output wire [7:0]Result_D_1,
	output wire [13:0]Img_A_1,
	output wire fin_1,
	output wire Img_CEN_1,
...
reg [13:0]pre_A;
reg [51:0]x;//p(x)
reg signed[153:0]pre_D;
wire [51:0]pre_x;//pre_p(x)
wire [7:0]real_x, real_y;//coordinates of the required data in ROM 
wire signed[11:0]a, b, c, d;
wire signed[99:0] x_sq;
wire signed[147:0] x_cu;
...
assign Result_D_1 = pre_D[152:145];
assign Img_A_1 = pre_A;
assign fin_1 = pre_fin;
assign Img_CEN_1 = pre_CEN;
...
assign pre_x = ({original_h-8'd1, 48'd0}/(scaled_h-8'd1))*(current_addr/scaled_w);
	...
	if(rst)begin
		...
		case(CS)
			...
			waiting:begin//find 0, detect whether next(-1) is bound
				if(real_y==8'd0) pre_A <= pre_A;
				else pre_A <= pre_A - 14'd128;
				CS <= col0;
			end
			col0:begin//find -1
				if(real_y==8'd0) pre_A <= pre_A + 14'd128;
				else pre_A <= pre_A + 14'd256;
				CS <= col1;
			end
			col1:begin//find 1, detect whether next(2) is bound
				if(real_y==8'd126) pre_A <= pre_A;
				else pre_A <= pre_A + 14'd128;
				CS <= col2;
			end
			col2:begin//find 2
			...
			calculate:begin
				pre_D <= a*x_cu+b*$signed({x_sq, 48'd0})+c*$signed({$signed(x), 96'd0})+$signed({d, 144'd0});
				CS <= round;
			end
			round:begin
				if(pre_D>$signed({9'd255, 145'd0})) pre_D <= $signed({9'd255, 145'b0});
				else if(pre_D<$signed(0)) pre_D <= 154'd0;
				else begin
					if(pre_D[144]==1'b0) pre_D <= $signed({pre_D[153:145], 145'b0});
					else begin
						/*if(|pre_D[143:0]==1'b1)*/ pre_D <=$signed({$signed(pre_D[153:145])+$signed(9'd1), 145'b0});
						/*else if(pre_D[145]==1'b0) pre_D <= $signed({pre_D[153:145], 145'b0});
						else pre_D <= $signed({$signed(pre_D[153:145])+$signed(9'd1), 145'b0});*/
					end
				end
				CS <= write;
			end
